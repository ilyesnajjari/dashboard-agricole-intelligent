...
import { Box, Button, Grid, MenuItem, Select, Snackbar, Alert, TextField, Typography, Paper, Skeleton, ToggleButton, ToggleButtonGroup } from '@mui/material'
...
export default function AchatsPage() {
  ...
  const [view, setView] = useState<'merch'|'material'>('material')
  ...
  // Fetch broadly (date filters only); we split client-side by view
  ...
  const materialRows = items.filter(it => it.category === 'tools' || it.category === 'inputs')
  const merchRows = items.filter(it => it.category === 'other')
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      const payload = view === 'merch' ? { ...form, category: 'other' as any } : form
    ...
    <Box p={3}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
        ...
        <ToggleButtonGroup size="small" exclusive value={view} onChange={(_, v) => { if (v) setView(v) }}>
          <ToggleButton value="material">Matériel (outils, intrants)</ToggleButton>
          <ToggleButton value="merch">Marchandises (pour revente)</ToggleButton>
        </ToggleButtonGroup>
      </Box>
      ...
      <Box mt={3} component={Paper} sx={{ p:2 }}>
        <Typography variant="h6" gutterBottom>Ajouter un achat ({view === 'material' ? 'Matériel' : 'Marchandise'})</Typography>
        <Grid container spacing={2} alignItems="center" component="form" onSubmit={onSubmit}>
          ...
          {view === 'material' ? (
            ...
            <Grid item xs={12} md={3}>
              <TextField fullWidth label="Type" value="Marchandise" disabled />
            </Grid>
          )}
          ...
          <Grid item xs={12} md={2}><TextField fullWidth type="number" inputProps={{ step: '0.01' }} label="Montant (€)" value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} required /></Grid>
          <Grid item xs={12} md={2}><Button type="submit" variant="contained">Enregistrer</Button></Grid>
        </Grid>
      </Box>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p:2 }}>
            <Typography variant="h6" gutterBottom>
              {view === 'material' ? 'Matériel (outils, intrants)' : 'Marchandises (pour revente)'}
            </Typography>
            <div style={{ width: '100%' }}>
              {loading ? <Skeleton variant="rounded" width="100%" height={280} /> : (
              <DataGrid
                rows={view === 'material' ? materialRows.filter(r => !category || r.category === category) : merchRows}
                columns={columns}
                getRowId={(r) => r.id!}
                autoHeight
