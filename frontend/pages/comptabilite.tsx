...
import { AccountingEntry, createEntry, deleteEntry, listEntries, updateEntry } from '../api/accounting'
...
export default function ComptabilitePage() {
  ...
  const [form, setForm] = useState<AccountingEntry>({ date: new Date().toISOString().slice(0,10), type: 'expense', label: '', amount: 0 })
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setErr(e?.message ?? 'Erreur chargement comptabilité')
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'type', headerName: 'Type', flex: 1, editable: true, type: 'singleSelect', valueOptions: ['income','expense'] },
    { field: 'label', headerName: 'Libellé', flex: 2, editable: true },
  ...
  const processRowUpdate = async (newRow: GridRowModel, oldRow: GridRowModel) => {
    try {
      const changed: any = {}
      ;(['type','label','amount'] as const).forEach(k => { if (newRow[k] !== oldRow[k]) changed[k] = newRow[k] })
      if (Object.keys(changed).length) {
        await updateEntry(Number(newRow.id), changed)
        setToast({open:true, msg:'Écriture mise à jour', severity:'success'})
        await refresh()
      }
      return newRow
    } catch (e: any) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ date: new Date().toISOString().slice(0,10), type: 'expense', label: '', amount: 0 })
      setToast({open:true, msg:'Écriture ajoutée', severity:'success'})
      refresh()
    } catch (e: any) {
    ...
    <Box p={3}>
      ...
      <Grid container spacing={2} alignItems="center">
        ...
        <Grid item xs={12} md={3}>
          <Select fullWidth value={type} displayEmpty onChange={(e) => setType(e.target.value as any)}>
      ...
      <Box mt={3} component={Paper} sx={{ p:2 }}>
        <Typography variant="h6" gutterBottom>Ajouter une écriture</Typography>
        <Grid container spacing={2} alignItems="center" component="form" onSubmit={onSubmit}>
          ...
          <Grid item xs={12} md={3}>
            <Select fullWidth value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value as any })}>
              <MenuItem value="income">Revenu</MenuItem>
              <MenuItem value="expense">Dépense</MenuItem>
            </Select>
          </Grid>
          <Grid item xs={12} md={4}><TextField fullWidth label="Libellé" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} /></Grid>
      ...
      <Grid container spacing={2} mt={1}>
        <Grid item xs={12}>
          <Paper sx={{ p:2 }}>
            <Typography variant="h6" gutterBottom>Table des écritures</Typography>
            <div style={{ width: '100%' }}>
